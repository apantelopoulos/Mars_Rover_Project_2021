...
//for timing when data sent to control, without slowing down whole loop by adding delays
int count = 0;
...
float radius = 145; // measured from middle of sensor LED to middle of rover - radius of rotation; tweak for more accurate angle performance
...
int total_x = 0;
int total_y = 0;
int total_x_init = 0; //last actual_total_x before resetting to 0, to avoid 'wrap-around'
int total_y_init = 0;
int actual_total_x = 0; //to circumvent 20cm 'wrap-around' effect, need new var to track actual total
int actual_total_y = 0;
...
void loop() {
  ...
  if (command=="forward" || command=="back" || command=="right" || command=="left" || command=="stop" || command=="reset") { //hm, idk about the last two...
    init_x = actual_total_x;
    init_y = actual_total_y;
    ...
    }else if (command=="reset"){ // Should this stop the rover? Should the current command abort?
      actual_x = 0;
      actual_y = 0;
      actual_angle = 0;
    }
  ...
  if (abs(total_y) < 200){
    actual_total_y = total_y_init + total_y;
  }else{
    total_y_init = actual_total_y;
    total_y1 = 0; // this is the og variable (counts per inch) based on which total_y is calculated 
  }

  if (abs(total_x) < 200){
    actual_total_x = total_x_init + total_x;
  }else{
    total_x_init = actual_total_x;
    total_x1 = 0; // this is the og variable (counts per inch) based on which total_y is calculated 
  }


  dist_from_init = actual_total_y-init_y; // this means dist_from_init=0 during rotations; is true of centre of rover

  //measured from middle of sensor LED to middle of rover - radius of rotation; tweak for more accurate angle performance
  //can calculate by manually rotating 90deg; change in total_x is circumference/4 = 2*pi*r/4
  radius = 145;

  angle_from_init = 360*(init_x-actual_total_x)/(2*PI*radius); //init_x-actual_total_x so that right is +ve
  //now this works for >90deg; assumes sensor's x-axis orientation always tangential to it, so change in x is length of arc
  

  if (current_status=="going forward" || current_status=="going back") {
    angle_init = actual_angle;
    //actual_angle = angle_init
    actual_x = actual_x_init + dist_from_init * sin(radians(actual_angle));
    actual_y = actual_y_init + dist_from_init * cos(radians(actual_angle));
  ...
  //this (used in if-statement below) ensures data is being sent at moderate intervals even when no changes to coords/status
  if (count <= 30) {
    count++;
  } else {
    count = 0;
...
//|| current_status!=prev_status || actual_x!=prev_actual_x || actual_y!=prev_actual_y
  //this if-statement sends data only on a moderate-time count (to avoid data corruption if send too fast)
  if ( count==10) {
    ...
    Serial.println("actual_angle = " + String(actual_angle));
    Serial.println("");
    ...
    Serial1.print(",");
    Serial1.print(String(actual_x));
    Serial1.print(",");
    Serial1.print(String(actual_y));
    Serial1.print(",");
    Serial1.print(String(actual_angle));
    Serial1.print(",");
    Serial1.print(String(current_status_encoded));
    Serial1.print("E");
    //Serial1.println(",");
    //Serial1.println("1");
    //Serial1.println("-1000");

//    prev_actual_x = actual_x;
//    prev_actual_y = actual_y;
