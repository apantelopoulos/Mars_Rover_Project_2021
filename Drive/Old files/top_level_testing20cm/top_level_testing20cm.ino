...
void loop() {
  ...
  */
  ...
  if (count < 500) {
    ...
    command = "right"; //change this to change the command tested
    magnitude = 360; //''
  ...
  dist_from_init = actual_total_y-init_y; // this means dist_from_init=0 during rotations (unlike previously, where changing x affected dist); is true of centre of rover!
  ...
  angle_from_init = 360*(init_x-actual_total_x)/(2*PI*radius); //init_x-total_x so that right is +ve
