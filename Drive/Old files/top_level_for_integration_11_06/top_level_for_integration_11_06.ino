...
float radius = 130; // measured from middle of sensor LED to middle of rover - radius of rotation; tweak for more accurate angle performance
...
//  radius = 130;
  ...
  //this if-statement sends data only upon 'events' or on a moderate-time count (to avoid data corruption if send too fast)
  if ( (prev_actual_x != actual_x) || (prev_actual_y != actual_y) || (prev_status != current_status) || (count==10) ) {
